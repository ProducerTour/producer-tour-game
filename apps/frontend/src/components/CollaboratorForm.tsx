    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm">
          <span className={`font-medium ${isValidSplit ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="space-y-3">
        {collaborators.map((collab, index) => (
          <div key={collab.id || index} className="bg-slate-800 rounded-lg p-4 border border-slate-700 relative">
            {collab.isPrimary && (
              <div className="absolute top-2 right-2">
                <span className="px-2 py-1 bg-purple-600 text-white text-xs rounded">Primary</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-900 border border-slate-600 rounded text-white focus:ring-2 focus:ring-purple-500"
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                    {suggestions.map((suggestion, i) => (
                      <button
                        ...
                        className="w-full px-3 py-2 text-left hover:bg-slate-700 text-white text-sm flex justify-between items-center"
                      >
                        <span>
                          {suggestion.fullName} ({suggestion.role})
                        </span>
                        <span className="text-gray-400 text-xs">{suggestion.frequency}x</span>
                      </button>
                    ))}
                  </div>
                )}
              ...
              {!collab.isPrimary && (
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded font-medium transition-colors"
                  >
                    Remove
                  </button>
                </div>
      ...
      {!isValidSplit && (
        <div className="bg-red-900/20 border border-red-500 rounded-lg p-3 text-red-400 text-sm">
          ⚠️ Total split percentage cannot exceed 100%. Please adjust the values.
        </div>
      )}
    </div>
  );
