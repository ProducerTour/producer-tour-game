 ...
 * Moved from lib/city/ to lib/terrain/ during cleanup
 */
...
export class NoiseGenerator {
  ...
  fbm2(
    x: number,
    y: number,
    octaves: number = NOISE_CONFIG.octaves,
    persistence: number = NOISE_CONFIG.persistence,
    lacunarity: number = NOISE_CONFIG.lacunarity,
    scale: number = NOISE_CONFIG.baseScale
  ): number {
    let value = 0;
    let amplitude = 1;
    let frequency = scale;
  ...
  getTerrainHeight(worldX: number, worldZ: number, districtBias: number = 0): number {
    ...
    // Combine and scale
    const height = (baseHeight + ridgeHeight) * MAX_HEIGHT + districtBias;
