...
type Producer = {
  ...
  ipRoyaltyRate: number;
  trustSweepRate: number;
};

type Assumptions = {
  corpType: "S-Corp" | "C-Corp";
  payrollTaxRate: number;
  ownerEffectiveIncomeTaxRate: number;
  llcProfitEffectiveTaxRate: number;
  holdingsStateTaxRate: number;
  trustDistributionTaxRate: number;
};

export default function TrifectaPlannerPage() {
  const [assumptions, setAssumptions] = useState<Assumptions>({
    corpType: "S-Corp",
    payrollTaxRate: 0.0765 + 0.0765,
    ownerEffectiveIncomeTaxRate: 0.24,
    llcProfitEffectiveTaxRate: 0.0,
    holdingsStateTaxRate: 0.0,
    trustDistributionTaxRate: 0.05,
  });
  ...
  function computeProducer(p: Producer) {
    ...
    const payrollTaxes = p.salary * assumptions.payrollTaxRate;
    ...
    const ownerTaxableIncome = p.salary + llcAfterTaxProfit;
    const ownerIncomeTax = ownerTaxableIncome * assumptions.ownerEffectiveIncomeTaxRate;
    const holdingsGross = royaltyToHoldings;
  ...
  const onChangeAssumption = (k: keyof Assumptions, v: number | string) => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto text-slate-100 min-h-screen bg-slate-950">
      <motion.h1 initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-semibold mb-2 text-sky-400">
        Producer Tour · Interactive Trifecta Planner
      </motion.h1>
      <p className="text-white/90 mb-6">Model the flow from <b>Operating LLC</b> → <b>Holdings (IP)</b> → <b>Family Trust/Foundation</b> per producer. Adjust the knobs and watch the math update. All rates are editable to match your CPA guidance.</p>
      ...
      <Tabs defaultValue="producers" className="space-y-6">
        ...
        <TabsContent value="producers" className="space-y-6">
          <div className="flex items-center gap-3">
            ...
            <label className="inline-flex items-center gap-2 px-3 py-2 bg-slate-800 rounded-xl cursor-pointer hover:bg-slate-700 transition-colors">
                  ...
                  <CardContent className="p-5 space-y-4">
                    <div className="flex items-center justify-between">
                      <Input
                        ...
                        className="text-xl font-semibold max-w-[60%] bg-slate-800 text-white border-slate-600"
                      />
          ...
          <Card className="bg-slate-900/60 border border-slate-800">
            <CardContent className="p-5 space-y-4">
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <Label className="text-white">Corp Type</Label>
                  <select
                    value={assumptions.corpType}
                    onChange={(e) => onChangeAssumption("corpType", e.target.value)}
                    className="w-full mt-2 bg-slate-800 rounded-xl px-3 py-2 text-white border border-slate-600"
                  >
              ...
              <p className="text-sm text-white/70">All rates are effective scenario dials. For bracket-accurate projections, export and share with your CPA.</p>
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <Card className="bg-slate-900/60 border border-slate-800 lg:col-span-2">
              <CardContent className="p-5">
                <h3 className="mb-3 font-semibold text-white">Per-Producer Flow</h3>
                <div className="h-72">
            ...
            <Card className="bg-slate-900/60 border border-slate-800">
              <CardContent className="p-5">
                <h3 className="mb-3 font-semibold text-white">Tax Mix (All Producers)</h3>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
        ...
        <TabsContent value="summary" className="space-y-6">
          <Card className="bg-slate-900/60 border border-slate-800">
            <CardContent className="p-5 space-y-3">
              ...
              <p className="text-sm text-white/70">* Net Benefit to Owner = LLC after-tax profit − owner income tax + (Holdings retained + Trust after-tax). Adjust assumptions to reflect your CPA's plan.</p>
            </CardContent>
          </Card>

          <Card className="bg-slate-900/60 border border-slate-800">
            <CardContent className="p-5 space-y-3">
              <h3 className="font-semibold text-white">Compliance Checklist (Kohler-style)</h3>
...
function FormNumber({ label, value, onChange, step = 1000, min = 0, max }: { label: string; value: number; onChange: (v: number) => void; step?: number; min?: number; max?: number; }) {
  return (
    <div>
      ...
      <Input
        type="number"
        className="mt-2 bg-slate-800 text-white border-slate-600"
