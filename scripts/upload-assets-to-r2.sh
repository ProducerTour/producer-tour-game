...
# Upload base avatars (assets folder)
if [ -d "./assets" ]; then
    echo "Uploading assets (base avatars)..."
    aws s3 sync ./assets s3://$BUCKET_NAME/assets \
        --endpoint-url $R2_ENDPOINT \
        --delete
    echo "  Done."
else
    echo "Skipping assets (directory not found)"
fi

# Upload icons
if [ -d "./icons" ]; then
    echo "Uploading icons..."
    aws s3 sync ./icons s3://$BUCKET_NAME/icons \
        --endpoint-url $R2_ENDPOINT \
        --delete
    echo "  Done."
else
    echo "Skipping icons (directory not found)"
fi
